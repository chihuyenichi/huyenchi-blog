...
import { displayCategory, getAllPosts, getPost, getRelatedPosts, getRenderedMarkdown } from "@/lib/content";
...
export default async function WriteupPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <article className="writeup"><CopyLinkScript />
    <div className="shell article-shell">
      ...
      <div className="article-layout"><aside className="article-aside"><p>ON THIS PAGE</p><a href="#writeup">Writeup</a><a href="#labels">Labels</a></aside><div className="prose" id="writeup"><ReactMarkdown remarkPlugins={[remarkGfm, remarkMath]} rehypePlugins={[rehypeKatex]}>{getRenderedMarkdown(post)}</ReactMarkdown></div></div>
      ...
      {related.length > 0 && <section className="related"><div className="section-heading"><div><p className="kicker">KEEP READING</p><h2>Other writeups of interest</h2></div></div><div className="writeup-list">{related.map((item) => <WriteupRow post={item} key={item.slug} />)}</div></section>}
    </div>
  </article>;
}
